...
def main():
    ...
    global ct, channel1, channel2, pause, smb, tloz, line_clear_sound, bump, end, gameover, ab, tetris, gta_4, ff, tes_4_o, f_3, h, h_3, tes_5_s_d, tes_5_s_f, p_1, p_2, m, tw_3_wh, INPUT_DEVICE
    ...
    life = 3
    showTextScreen('Quatris', True)
    file = open('ranking.txt', 'w')
    while life >= 0: # game loop
        global music, level, score
        ...
        runGame(life)
        file.write(str(life) + ' ' + str(level) + ' ' + str(score) + '\n')
        channel1.stop()
        channel2.play(end, 0)
        showTextScreen('Game Over', False)
        life -= 1
    file.close()
    DISPLAYSURF.fill(BGCOLOR)
    channel2.play(gameover, 0)
    showTextScreen('Insert Coin', False)  # pause until a key press
...
def runGame(life):
    ...
    dirList = [0, -1, 0, 1]
    dirIndex = 0
    direction = [dirList[dirIndex], dirList[len(dirList) - 1 - dirIndex]]
    ...
    fallingPiece = getNewPiece(direction)
    nextPiece = getNewPiece([-1, 0])
    dirIndex += 1
    next_direction = [dirList[dirIndex], dirList[len(dirList) - 1 - dirIndex]]
            ...
            for event in pygame.event.get():  # event handling
                if event.type == KEYUP:
                    if event.key == K_p:
                        ...
                        channel1.pause()
                        channel2.play(pause)
                        showTextScreen('Paused', True)  # pause until a key press
                        channel1.unpause()
                        lastFallTime = time.time()
                        lastMoveDownTime = time.time()
                        lastMoveSidewaysTime = time.time()
...
def showTextScreen(text, key):
    ...
    if key or INPUT_DEVICE:
        ...
        pressKeySurf, pressKeyRect = makeTextObjs('Press select key to continue.', BASICFONT, TEXTCOLOR)
        pressKeyRect.center = (int(WINDOWWIDTH / 2), int(WINDOWHEIGHT / 2) + 100)
        DISPLAYSURF.blit(pressKeySurf, pressKeyRect)

        while checkForSelectPress() == None:
            pygame.display.update()
            FPSCLOCK.tick()
...
def drawStatus(score, level, life):
    ...
    inputSurf = BASICFONT.render('Input: %s' % input, True, TEXTCOLOR)
    inputRect = inputSurf.get_rect()
    inputRect.topleft = (WINDOWWIDTH - 300, 230)
    DISPLAYSURF.blit(inputSurf, inputRect)
    ...
    controlSurf = BASICFONT.render('%s : rotate' % rotate, True, TEXTCOLOR)
    controlRect = controlSurf.get_rect()
    controlRect.topleft = (WINDOWWIDTH - 300, 300)
    DISPLAYSURF.blit(controlSurf, controlRect)

    control1Surf = BASICFONT.render('%s : drop' % drop, True, TEXTCOLOR)
    control1Rect = control1Surf.get_rect()
    control1Rect.topleft = (WINDOWWIDTH - 300, 330)
    DISPLAYSURF.blit(control1Surf, control1Rect)

    control2Surf = BASICFONT.render('%s : move' % move, True, TEXTCOLOR)
    control2Rect = control2Surf.get_rect()
    control2Rect.topleft = (WINDOWWIDTH - 300, 360)
    DISPLAYSURF.blit(control2Surf, control2Rect)

    control3Surf = BASICFONT.render('%s : pause' % pause, True, TEXTCOLOR)
    control3Rect = control3Surf.get_rect()
    control3Rect.topleft = (WINDOWWIDTH - 300, 390)
    DISPLAYSURF.blit(control3Surf, control3Rect)

    control4Surf = BASICFONT.render('%s : exit' % exit, True, TEXTCOLOR)
    control4Rect = control4Surf.get_rect()
    control4Rect.topleft = (WINDOWWIDTH - 300, 420)
    DISPLAYSURF.blit(control4Surf, control4Rect)
